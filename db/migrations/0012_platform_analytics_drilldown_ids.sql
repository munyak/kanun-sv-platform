-- 0012 — Platform analytics drill-down: add monitor user_id/org_id to
-- platform_admin_usage_analytics.monitor_activity so the admin analytics
-- rows can open the existing user/org detail panels. (org_usage already
-- returned org_id.) Applied to project yxhwcicxarfmptwivkdu 2026-06-29.
  ...
  select jsonb_build_object(
    ...
    'visits_summary', (
      select jsonb_build_object(
        ...
        'cancelled',  count(*) filter (where status in ('canceled_custodial','canceled_noncustodial','canceled_provider')),
        'no_show',    count(*) filter (where status in ('no_show_custodial','no_show_noncustodial')),
        'in_progress', count(*) filter (where status in ('checked_in','in_progress')),
    ...
    'daily_visits', (
      ...
      from (
        select
          ...
          count(*) filter (where status in ('canceled_custodial','canceled_noncustodial','canceled_provider')) as cancelled
        from public.sv_visits
        where scheduled_date >= v_start
    ...
    'monitor_activity', (
      ...
      from (
        select
          ...
          mon.user_id as user_id,
          mon.org_id as org_id,
          mon.first_name,
          mon.last_name,
          o.name as org_name,
        ...
        group by mon.id, mon.user_id, mon.org_id, mon.first_name, mon.last_name, o.name
        limit 50
      ) m
    ),
